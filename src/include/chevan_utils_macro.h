...
#define _IF_ELSE0(...) CLEAR(__VA_ARGS__) _EVAL1
...
#define EVAL(...) _EVAL256(__VA_ARGS__)
// how to use: IF_ELSE(1)(smt)(smt)
#define IF_ELSE(C) _EVAL32(_IF_ELSE(BOOL(C)))
...
#define _ENUM_PARSE_N(val, loc, A, ...) \
  ...
  ((!strcmp(val, #A) ? loc A : _DEFER2(_ENUM_PARSE_N_)()(val, loc, __VA_ARGS__)))(loc A)
